local ui_options = {
local main_color = Color3.fromRGB(150, 1, 1),
...
Strings.ZIndex = 5 Strings.Font = Enum.Font.Code
Strings.Text = ""
...
MultiDropdownButton.Parent = Prefabs
MultiDropdownButton.BackgroundColor3 = Color3.new(0.129412, 0.133333, 0.141176)
MultiDropdownButton.BorderSizePixel = 0
MultiDropdownButton.Size = UDim2.new(1, 0, 0, 20)
MultiDropdownButton.ZIndex = 3
MultiDropdownButton.Font = Enum.Font.Oswald
MultiDropdownButton.Text = " Button"
MultiDropdownButton.TextColor3 = Color3.new(0.784314, 0.784314, 0.784314)
MultiDropdownButton.TextSize = 18
MultiDropdownButton.TextXAlignment = Enum.TextXAlignment.Left
Checkmark.Name = "Checkmark"
Checkmark.Parent = MultiDropdownButton
Checkmark.BackgroundColor3 = Color3.new(1, 1, 1)
Checkmark.BackgroundTransparency = 1
Checkmark.Position = UDim2.new(0, 8, 0, 0)
...
local function ripple(button, x, y)
    spawn(function()
        ...
        elseif button.AbsoluteSize.X < button.AbsoluteSize.Y then
            local size = button.AbsoluteSize.Y * 1.5
        elseif button.AbsoluteSize.X == button.AbsoluteSize.Y then
            local size = button.AbsoluteSize.X * 1.5
        end
...
local function format_windows()
    ...
    function library:AddWindow(title, options)
        local windows = windows + 1
        local dropdown_open = false
        local title = tostring(title or "New Window")
        local options = (typeof(options) == "table") and options or ui_options
                    ...
                    spawn(function()
                        if Entered and Resizer.Active and options.can_resize then
                            while Held and Resizer.Active do
                                local mouse_location = getInputPosition(inputObject)
        ...
        do
            ...
            do
                function window_data:AddTab(tab_name)
                    local tab_data = {}
                    local tab_name = tostring(tab_name or "New Tab")
                    ...
                    do
                        function tab_data:AddLabel(label_text)
                            local label_text = tostring(label_text or "New Label")
                        ...
                        function tab_data:AddButton(button_text, callback)
                            local button_text = tostring(button_text or "New Button")
                        ...
                        function tab_data:AddSwitch(switch_text, callback)
                            local switch_data = {}
                            local switch_text = tostring(switch_text or "New Switch")
                            local callback = typeof(callback) == "function" and callback or function()end
                            ...
                            switch.MouseButton1Click:Connect(function()
                                local toggled = not toggled
                                switch.Text = toggled and utf8.char(10003) or ""
                                pcall(callback, toggled)
                            end)
                            function switch_data:Set(bool)
                                local toggled = (typeof(bool) == "boolean") and bool or false
                                switch.Text = toggled and utf8.char(10003) or ""
                                pcall(callback,toggled)
                            end
                        ...
                        function tab_data:AddTextBox(textbox_text, callback, textbox_options)
                            local textbox_text = tostring(textbox_text or "New TextBox")
                            local callback = typeof(callback) == "function" and callback or function()end
                            local textbox_options = typeof(textbox_options) == "table" and textbox_options or {["clear"] = true}
                        ...
                        function tab_data:AddSlider(slider_text, callback, slider_options)
                            local slider_data = {}
                            local slider_text = tostring(slider_text or "New Slider")
                            local callback = typeof(callback) == "function" and callback or function()end
                            local slider_options = typeof(slider_options) == "table" and slider_options or {}
                            ...
                            do
                                ...
                                local function updateSlider(input)
                                    if not slider_options.readonly and (Held or Entered) and not dropdown_open then
                                        local inputPos = getInputPosition(input)
                                        local x = (slider.AbsoluteSize.X - (slider.AbsoluteSize.X - ((inputPos.X - slider.AbsolutePosition.X)) + 1)) / slider.AbsoluteSize.X
                                        local min = 0
                                        local max = 1
                                ...
                                UIS.InputBegan:Connect(function(inputObject)
                                    if inputObject.UserInputType == Enum.UserInputType.MouseButton1 or inputObject.UserInputType == Enum.UserInputType.Touch then
                                        if Entered and not slider_options.readonly then
                                            ...
                                            local currentConnection = RS.Heartbeat:Connect(function()
                                                if Held then
                                ...
                                UIS.InputChanged:Connect(function(inputObject)
                                    if inputObject.UserInputType == Enum.UserInputType.Touch and Held then
                                        updateSlider(inputObject)
                                    end
                                end)
                                UIS.InputEnded:Connect(function(inputObject)
                                ...
                                function slider_data:Set(new_value)
                                    local new_value = tonumber(new_value) or 0
                                    local new_value = (((new_value >= 0 and new_value <= 100) and new_value) / 100)
                        ...
                        function tab_data:AddKeybind(keybind_name, callback, keybind_options)
                            local keybind_data = {}
                            local keybind_name = tostring(keybind_name or "New Keybind")
                            local callback = typeof(callback) == "function" and callback or function()end
                            local keybind_options = typeof(keybind_options) == "table" and keybind_options or {}
                            ...
                            local shortkeys = {
                            local RightControl = 'RightCtrl',
                            local LeftControl = 'LeftCtrl',
                            local LeftShift = 'LShift',
                            local RightShift = 'RShift',
                            local MouseButton1 = "Mouse1",
                            local MouseButton2 = "Mouse2"
                            }
                        ...
                        function tab_data:AddDropdown(dropdown_name, callback)
                            local dropdown_data = {}
                            local dropdown_name = tostring(dropdown_name or "New Dropdown")
                            local callback = typeof(callback) == "function" and callback or function()end
                            ...
                            function dropdown_data:Add(n)
                                local object_data = {}
                                local n = tostring(n or "New Object")
                        ...
                        function tab_data:AddMultiDropdown(dropdown_name, callback)
                            local dropdown_data = {}
                            local dropdown_name = tostring(dropdown_name or "New Multi Dropdown")
                            local callback = typeof(callback) == "function" and callback or function()end
                            local dropdown = Prefabs:FindFirstChild("Dropdown"):Clone()
                            local box = dropdown:FindFirstChild("Box")
                            local objects = box:FindFirstChild("Objects")
                            ...
                            local function updateDisplayText()
                                ...
                                elseif #selected_items == 1 then
                                    dropdown.Text = " [ " .. selected_items[1] .. " ]"
                                else
                                    dropdown.Text = " [ " .. #selected_items .. " selected ]"
                                end
                            end
                            dropdown.MouseButton1Click:Connect(function()
                                local open = not open
                            ...
                            function dropdown_data:Add(n)
                                local object_data = {}
                                local n = tostring(n or "New Object")
                                local object = Prefabs:FindFirstChild("MultiDropdownButton"):Clone()
                                ...
                                object.MouseEnter:Connect(function()
                                    if not is_selected then
                                        object.BackgroundColor3 = Color3.fromRGB(45, 46, 48)
                                    end
                                end)
                                object.MouseLeave:Connect(function()
                                    if not is_selected then
                                ...
                                object.MouseButton1Click:Connect(function()
                                    if dropdown_open then
                                        local is_selected = not is_selected
                                        if is_selected then
                                            checkmark.Text = "✓"
                                            object.BackgroundColor3 = options.main_color
                                            table.insert(selected_items, n)
                                        else
                                            checkmark.Text = ""
                                            object.BackgroundColor3 = Color3.fromRGB(33, 34, 36)
                                            for i, v in ipairs(selected_items) do
                                                if v == n then
                                ...
                                function object_data:SetSelected(bool)
                                    local bool = typeof(bool) == "boolean" and bool or false
                                    if bool and not is_selected then
                                        ...
                                        updateDisplayText()
                                        pcall(callback, selected_items)
                                    elseif not bool and is_selected then
                                        local is_selected = false
                                        checkmark.Text = ""
                                        object.BackgroundColor3 = Color3.fromRGB(33, 34, 36)
                                        for i, v in ipairs(selected_items) do
                            ...
                            function dropdown_data:ClearAll()
                                local selected_items = {}
                                for _, object in pairs(objects:GetChildren()) do
                                    if object:IsA("TextButton") and object.Name == "MultiDropdownButton" then
                                        object:FindFirstChild("Checkmark").Text = ""
                                        object.BackgroundColor3 = Color3.fromRGB(33, 34, 36)
                                    end
                                end
                                updateDisplayText()
                                    ...
                                    if inputObject.UserInputType == Enum.UserInputType.MouseButton1 or inputObject.UserInputType == Enum.UserInputType.Touch then
                                        ...
                                        spawn(function()
                                            while Held and Entered1 and (not dropdown_open) do
                                                local inputPos = getInputPosition(inputObject)
                                                local x = ((palette.AbsoluteSize.X - (inputPos.X - palette.AbsolutePosition.X)) + 1)
                                                local y = ((palette.AbsoluteSize.Y - (inputPos.Y - palette.AbsolutePosition.Y)) + 1.5)
                        ...
                        function tab_data:AddConsole(console_options)
                            local console_data = {}
                            local console_options = typeof(console_options) == "table" and console_options or {["readonly"] = true,["full"] = false,}
                            ...
                            do
                                ...
                                local strings = function(string)
                                    ...
                                    string:gsub(".", function(c)
                                        ...
                                        if quote == false and c == "\34" then
                                            local highlight = highlight .. "\34"
                                        elseif c == "\n" then
                                            local highlight = highlight .. "\n"
                                        elseif c == "\t" then
                                            local highlight = highlight .. "\t"
                                        elseif quote == true then
                                            local highlight = highlight .. c
                                        elseif quote == false then
                                            local highlight = highlight .. "\32"
                                        end
                                    end)
                                    return highlight
                                end
                        ...
                        function tab_data:AddFolder(folder_name)
                            local folder_data = {}
                            local folder_name = tostring(folder_name or "New Folder")
